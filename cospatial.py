...
@njit(fastmath=True)
def make_model(freqs1, freqs2, freqs3, freqs4, ints1, ints2, ints3, ints4, ss, datagrid0, datagrid1, vlsr1, vlsr2, vlsr3, vlsr4, dV, Tex):
    ...
    curr_model = apply_beam(datagrid0, (J_T - J_Tbg)*(1 - np.exp(-curr_model)), ss, 100)
...
def lnlike(theta, datagrid, mol_cat):
    ...
    source_size, Ncol1, Ncol2, Ncol3, Ncol4, Tex, vlsr1, vlsr2, vlsr3, vlsr4, dV = theta
    freqs1, ints1, taus1 = predict_intensities(source_size, Ncol1, Tex, dV, mol_cat)
    freqs2, ints2, taus2 = predict_intensities(source_size, Ncol2, Tex, dV, mol_cat)
    freqs3, ints3, taus3 = predict_intensities(source_size, Ncol3, Tex, dV, mol_cat)
    freqs4, ints4, taus4 = predict_intensities(source_size, Ncol4, Tex, dV, mol_cat)
    ...
    curr_model = make_model(freqs1, freqs2, freqs3, freqs4, taus1, taus2, taus3, taus4, source_size, datagrid[0], datagrid[1], vlsr1, vlsr2, vlsr3, vlsr4, dV, Tex)
...
def lnprior(theta, prior_stds, prior_means):
    source_size, Ncol1, Ncol2, Ncol3, Ncol4, Tex, vlsr1, vlsr2, vlsr3, vlsr4, dV = theta
    s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10 = prior_stds
    m0, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10 = prior_means
    ...
    if (0. < source_size < 500) and (0. < Ncol1 < 10**16.) and (0. < Ncol2 < 10**16.) and (0. < Ncol3 < 10**16.) and (0. < Ncol4 < 10**16.) and (vlsr1 < (vlsr2-0.05)) and (vlsr2 < (vlsr3-0.05)) and (vlsr3 < (vlsr4-0.05)) and (vlsr2 < (vlsr1+0.3)) and (vlsr3 < (vlsr2+0.2)) and (vlsr4 < (vlsr3+0.2)) and dV < 0.3:
        
        p0 = np.log(1.0/(np.sqrt(2*np.pi)*s0))-0.5*(source_size1-m0)**2/s0**2

        p5 = np.log(1.0/(np.sqrt(2*np.pi)*s5))-0.5*(Tex-m5)**2/s5**2

        p6 = np.log(1.0/(np.sqrt(2*np.pi)*s6))-0.5*(vlsr1-m6)**2/s6**2
        p7 = np.log(1.0/(np.sqrt(2*np.pi)*s7))-0.5*(vlsr2-m7)**2/s7**2
        p8 = np.log(1.0/(np.sqrt(2*np.pi)*s8))-0.5*(vlsr3-m8)**2/s8**2
        p9 = np.log(1.0/(np.sqrt(2*np.pi)*s9))-0.5*(vlsr4-m9)**2/s9**2

        p10 = np.log(1.0/(np.sqrt(2*np.pi)*s10))-0.5*(dV-m10)**2/s10**2

        return p0 + p5 + p6 + p7 + p8 + p9 + p10
...
def fit_multi_gaussian(datafile, fit_folder, prior_path, catalogue, nruns, restart=True):
    ...
    initial = [42.8, 5.8e12, 9.5e12, 4.e12, 1.06e13, 7.7, 5.603, 5.745, 5.873, 6.024, 0.1568]

    if restart==True:
        pos = [initial + np.array([1.e-1, 1.e10,1.e10,1.e10,1.e10, 1.e-3, 1.e-3,1.e-3,1.e-3,1.e-3, 1.e-3])*np.random.randn(ndim) for i in range(nwalkers)]
    else:
        initial = np.percentile(np.load(fit_folder + "/chain.npy")[:,-200:,:].reshape(-1,11).T, 50, axis=1)
        pos = [initial + np.array([1.e-1, 1.e10,1.e10,1.e10,1.e10, 1.e-3, 1.e-3,1.e-3,1.e-3,1.e-3, 1.e-3])*np.random.randn(ndim) for i in range(nwalkers)]
