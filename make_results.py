...
chain = np.load("chain_pt2.npy")

samples = np.load("chain_pt2.npy")[:,-400:,:].reshape(-1,14).T

#samples = samples[:,samples[13]<0.16]

print(np.percentile(samples,50, axis=1))

f= open("benzonitrile_formatted_results.txt","w+")
...
f= open("benzonitrile_results.txt","w+")

f.write("benzonitrile_hfs.cat\n")
...
fig.savefig("benzonitrile_corner.jpg")
